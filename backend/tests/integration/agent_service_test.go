...
func TestCreateAgent_Integration(t *testing.T) {
	...
	agent, err := env.AgentService.CreateAgent(env.Ctx, userID, name, description, dailyLimit)
...
func TestGetAgentByID_Integration(t *testing.T) {
	// Create test environment
	env := utils.NewTestEnv(t)
	defer env.Cleanup()
	...
	assert.Equal(t, userID, retrievedAgent.UserID)
}

func TestGetAgentsByUserID_Integration(t *testing.T) {
	// Create test environment
	env := utils.NewTestEnv(t)
	defer env.Cleanup()
	...
	for i := 0; i < 3; i++ {
		env.CreateTestAgent(userID)
	}

	// Test getting agents by user ID
	agents, err := env.AgentService.GetAgentsByUserID(env.Ctx, userID)
...
func TestRegenerateAPIKey_Integration(t *testing.T) {
	// Create test environment
	env := utils.NewTestEnv(t)
	defer env.Cleanup()
...
func TestIncrementUsage_Integration(t *testing.T) {
	// Create test environment
	env := utils.NewTestEnv(t)
	defer env.Cleanup()
...
func TestCheckRateLimit_Integration(t *testing.T) {
	...
	// Create a test agent with low daily limit
	agent := env.CreateTestAgent(userID)
	agent.DailyLimit = 2
	err := env.AgentService.UpdateAgent(env.Ctx, agent)
	require.NoError(t, err)
